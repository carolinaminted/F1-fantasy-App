...
readonly BUILD_MODE="production"
...
usage() {
  cat <<'EOF'
Usage: ./deploy-production.sh [--dry-run] [--expect-string <s>]

Builds the current `prod` commit in production mode and deploys it with NO traffic.
...
fail() { echo "Production deploy blocked: $*" >&2; exit 1; }
...
for required_file in package.json package-lock.json Dockerfile cloudbuild.web.yaml \
                     .env.production scripts/release-gates.sh scripts/bundle-audit.sh; do
...
[[ "$BUILD_MODE" == "production" ]] || fail "unexpected build mode"

# A production build with no portal callables URL silently falls back to formula-fantasy-1's own
# legacy functions — a backend no build has ever served. Cheap to check, expensive to discover
# after members are on it.
grep -q '^VITE_PORTAL_FUNCTIONS_BASE_URL=https://us-central1-lights-out-league-prod\.cloudfunctions\.net$' \
  .env.production || fail \
  ".env.production does not route callables to lights-out-league-prod.
  Without VITE_PORTAL_FUNCTIONS_BASE_URL the build falls back to formula-fantasy-1's legacy
  functions, which is not the backend that was validated."

# The badge renders whenever this is set. Production must not carry one.
! grep -q '^VITE_ENV_LABEL=' .env.production || fail \
  ".env.production sets VITE_ENV_LABEL — members would see an environment badge."

current_branch="$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo unknown)"
commit="$(git rev-parse HEAD)"
short_sha="$(git rev-parse --short HEAD)"
image_tag="production-${short_sha}"
revision_tag="p${short_sha}"

echo "Production deployment targets locked:"
echo "  GCP project:       $PROJECT"
echo "  Cloud Run service: $SERVICE ($REGION)"
echo "  Data plane:        formula-fantasy-1  ← PRODUCTION member data"
echo "  Callables:         lights-out-league-prod (Node 22 portal)"
echo "  Build mode:        $BUILD_MODE  ← no badge, indexable"
...
echo
echo "Building production image (Cloud Build, explicit tag)..."
...
audit_args=("$candidate_url" --profile production)
[[ -n "$expect_string" ]] && audit_args+=(--expect-string "$expect_string")
./scripts/bundle-audit.sh "${audit_args[@]}" || fail \
  "the candidate bundle failed its production audit. It has no traffic, so nothing is broken —
  but do not promote it. Inspect, fix, rebuild."

cat <<EOF

Production candidate deployed and audited. It is serving NO traffic.
