...
const RedFlagScreen: React.FC<RedFlagScreenProps> = ({ message }) => {
    ...
    return (
        <div data-theme="dark"
            className="fixed inset-0 z-[9999] bg-carbon-black overflow-hidden flex flex-col items-center justify-center font-exo text-ghost-white select-none">
            <style>
                {`
                    @keyframes floatUp {
            ...
            {particles.map(p => (
                <div 
                    key={p.id}
                    className="absolute w-1 h-1 bg-primary-red rounded-full pointer-events-none"
                    style={{
                        left: p.left,
            ...
            <div className="absolute w-full h-[2px] bg-primary-red opacity-20 pointer-events-none z-10" style={{ animation: 'scanline 6s linear infinite' }}></div>
            ...
            {/* Main Content Card */}
            <div className="relative z-20 bg-carbon-black border border-primary-red/30 p-8 md:p-12 rounded-2xl max-w-2xl w-[90%] flex flex-col items-center text-center shadow-[0_0_40px_rgba(218,41,28,0.2)]" style={{ animation: 'pulseGlow 4s ease-in-out infinite' }}>
                    ...
                    {[1, 2, 3, 4, 5].map(i => (
                        <div 
                            ...
                            className={`w-6 h-6 md:w-10 md:h-10 rounded-full border-2 border-gray-700 transition-all duration-75 ${
                                activeLight >= i 
                                ? 'bg-stage-alert shadow-[0_0_20px_var(--color-stage-alert)] scale-110 border-red-900' 
                                : 'bg-elev-2 opacity-30'
                            }`}
                        />
                    ))}
                </div>
                ...
                <div className="mb-6 relative w-24 h-24 md:w-32 md:h-32">
                    <svg viewBox="0 0 80 80" className="w-full h-full drop-shadow-[0_0_15px_rgba(218,41,28,0.5)]">
                        {/* Pole */}
                        <rect x="5" y="0" width="4" height="80" fill="var(--color-highlight-silver)" rx="2" />
                        ...
                        <defs>
                            <linearGradient id="flagGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                                <stop offset="0%" stopColor="var(--color-stage-ember)" />
                                <stop offset="50%" stopColor="var(--color-primary-red)" />
                                <stop offset="100%" stopColor="var(--color-stage-flare)" />
                            </linearGradient>
                        </defs>
                    </svg>
                </div>
                ...
                <div className="w-full bg-black/40 rounded-lg p-4 border border-white/5 font-mono text-xs md:text-sm space-y-2 text-left shadow-inner">
                    <div className="flex justify-between border-l-2 border-primary-red pl-3 bg-white/5 py-1">
                        <span className="text-highlight-silver/70">STATUS</span>
                        <span className="text-primary-red font-bold animate-pulse">RED FLAG</span>
                    </div>
            ...
            {/* Bottom Ticker */}
            <div className="fixed bottom-0 left-0 right-0 bg-primary-red/90 h-10 flex items-center overflow-hidden border-t border-primary-red z-30">
