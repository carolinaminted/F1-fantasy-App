...
#
# Usage: bundle-audit.sh <url> [--expect-string <s>] [--profile prod-staging|production]
#
# The profile decides two inverted checks. prod-staging (the default) must carry the noindex
# header and the PROD STAGING badge; production must carry neither. Everything else — the
# Firebase project, the callable host, staging leakage, credential assignments — is asserted
# identically for both, because those are what make the two builds the same application.
#
...
case "$profile" in
  ...
  *) echo "bundle-audit: --profile must be prod-staging or production (got '$profile')" >&2; exit 1 ;;
esac
...
echo "  Auditing $url (profile: $profile)"
...
# production build is the only one that strips it — so production asserts its ABSENCE. Shipping
# noindex to the live league would quietly de-index it; shipping an indexable prod-staging would
# put a duplicate of the league into search results.
robots_line="$( { grep -i '^x-robots-tag:' "$workdir/headers.txt" || true; } | tr -d '\r')"
if [[ "$profile" == "production" ]]; then
  if [[ -z "$robots_line" ]]; then
    pass "no X-Robots-Tag (production is indexable)"
  else
    flunk "production must not send X-Robots-Tag, but sent: $robots_line"
  fi
else
  if grep -qi '^x-robots-tag:[[:space:]]*noindex, nofollow, noarchive' "$workdir/headers.txt"; then
    ...
    flunk "X-Robots-Tag missing or wrong: ${robots_line:-(absent)}"
  fi
fi
...
check_absent() {
  local needle="$1" label="$2"
  if grep -qF "$needle" "$workdir/all.txt"; then flunk "$label — '$needle' IS present"; else pass "$label"; fi
}

check_present "formula-fantasy-1" "production Firebase project present"

# The badge renders only when VITE_ENV_LABEL is set, so its absence is what members should see.
if [[ "$profile" == "production" ]]; then
  check_absent "PROD STAGING" "no environment badge"
else
  check_present "PROD STAGING" "env label is PROD STAGING"
fi

# Both builds must reach the Node 22 portal callables. For production this is the check that
# catches a .env.production missing VITE_PORTAL_FUNCTIONS_BASE_URL, which would silently fall
# back to formula-fantasy-1's legacy functions.
